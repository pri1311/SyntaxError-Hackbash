...
import 'package:earthling/screens/suggestions.dart';
...
class _PMI_calculatorState extends State<PMI_calculator> {
  Map<String, dynamic> FoodandKitchen = {
    "Softdrink Bottles": 0,
    "Plastic Bags": 0,
    "Food Wrappers": 0,
    "Yoghurt Containers": 0,
  };
  Map<String, dynamic> bathroom = {
    "Cotton Swabs": 0,
    "Refill packets": 0,
    "Toothbrushes": 0,
    "Toothpastes": 0,
  ...
  Map<String, dynamic> containersandPackaging = {
  ...
  void saveMaps() async {
    ...
    var f = json.encode(FoodandKitchen);
    var b = json.encode(bathroom);
    var c = jsonEncode(containersandPackaging);
    prefs.setString('FoodandKitchen', f);
    prefs.setString('bathroom', b);
    prefs.setString('containersandPackaging', c);
  }

  void retrieve_maps() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String stringValue1 = prefs.getString('FoodandKitchen');
    String stringValue2 = prefs.getString('bathroom');
    String stringValue3 = prefs.getString('containersandPackaging');
    setState(() {
      if (stringValue1 != null) FoodandKitchen = json.decode(stringValue1);
      if (stringValue2 != null) bathroom = json.decode(stringValue2);
      if (stringValue3 != null)
        containersandPackaging = json.decode(stringValue3);
    });
  }
  ...
  getPMIValues() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    //Return double
    double pmi_r = prefs.getDouble('pmi');
    if (pmi_r != null) {
      pmi = pmi_r;
    } else {
      pmi = 0;
